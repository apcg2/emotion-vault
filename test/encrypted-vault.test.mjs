...
void test('sample import adds precisely seven prior days, is repeatable and preserves existing logs on quota failure', async () => {
  ...
  assert.deepEqual(
    restored.find((l) => l.id === sample().id),
    sample(),
  );
...
void test('new vault encrypts every sensitive field, appends while locked, and decrypts losslessly', async () => {
  const s = new MemoryStore();
...
void test('existing PIN gates migration; plaintext and old fast verifier removed only after verification', async () => {
...
void test('failed quota migration and cancellation preserve all old data', async () => {
  const s = await legacyStore();
  const before = new Map(s.data);
  s.failWrite = true;
...
void test('interrupted cleanup safely resumes after PIN; conflicting legacy writes are never removed', async () => {
  const s = await legacyStore();
  s.failRemove = true;
...
void test('concurrent appends retain all records and deletion retains unrelated records', async () => {
  const s = new MemoryStore();
  const session = await unlockVault(s, '0042', 'setup');
  await Promise.all(
...
void test('tampering with ciphertext or authenticated date refuses unlock without changing storage', async () => {
  const s = new MemoryStore();
  await unlockVault(s, '0042', 'setup');
  await appendLog(s, sample());
...
void test('malformed data does not reset the vault, and duplicate setup cannot overwrite keys', async () => {
  const s = new MemoryStore();
...
void test('legacy numeric IDs migrate without loss; invalid schemas are not overwritten', async () => {
  const s = new MemoryStore();
...
void test('encrypted backup restores losslessly into an empty store, including legacy vaults', async () => {
  const original = await legacyStore();
  await unlockVault(original, '0042', 'unlock');
  const before = new Map(original.data);
  const backup = exportEncryptedBackup(original);
  for (const secret of [
    '测试情境',
    '私密想法',
    '测试情绪',
  ...
  assert.equal(canRestoreBackup(destination), true);
  assert.equal(await restoreEncryptedBackup(destination, backup, '0042'), 1);
  assert.equal(canRestoreBackup(destination), false);
  assert.deepEqual((await unlockVault(destination, '0042', 'unlock')).logs, [
    sample(),
  ]);
  assert.equal(destination.getItem(LEGACY_LOG_KEY), null);
  assert.equal(destination.getItem(PRIVACY_PIN_KEY), null);
});

void test('backup rejects incomplete migration, unknown schemas, oversized input, wrong PIN and corruption', async () => {
  const original = await legacyStore();
  assert.throws(() => exportEncryptedBackup(original), /暂无加密数据/);
  original.failRemove = true;
  await assert.rejects(unlockVault(original, '0042', 'unlock'));
  assert.throws(() => exportEncryptedBackup(original), /完成旧数据迁移/);
  original.failRemove = false;
  await unlockVault(original, '0042', 'unlock');
  const backup = exportEncryptedBackup(original);
  const destination = new MemoryStore();
  for (const invalid of [
    '{',
    '{}',
    '{"format":"emotion-vault-backup","version":2}',
    JSON.stringify([sample()]),
  ])
    await assert.rejects(
      restoreEncryptedBackup(destination, invalid, '0042'),
      /有效/,
    );
  await assert.rejects(
    restoreEncryptedBackup(
      destination,
      'x'.repeat(BACKUP_MAX_BYTES + 1),
      '0042',
    ),
    /过大/,
  );
  await assert.rejects(
    restoreEncryptedBackup(destination, backup, '9999'),
    /密码不正确/,
  );
  await assert.rejects(
    restoreEncryptedBackup(destination, backup, '12'),
    /四位/,
  );
  const tampered = JSON.parse(backup);
  tampered.vault.records[0].data = 'AAAA';
  await assert.rejects(
    restoreEncryptedBackup(destination, JSON.stringify(tampered), '0042'),
    /解密校验失败/,
  );
  assert.equal(destination.data.size, 0);
});

void test('restore never overwrites existing or malformed data, even when only a PIN exists', async () => {
  const original = new MemoryStore();
  await unlockVault(original, '0042', 'setup');
  const backup = exportEncryptedBackup(original);
  for (const key of [VAULT_KEY, LEGACY_LOG_KEY, PRIVACY_PIN_KEY]) {
    const destination = new MemoryStore();
    destination.setItem(key, '{existing-malformed');
    const before = new Map(destination.data);
    await assert.rejects(
      restoreEncryptedBackup(destination, backup, '0042'),
      /不会覆盖/,
    );
    assert.deepEqual(destination.data, before);
  }
  await assert.rejects(
    restoreEncryptedBackup(original, backup, '0042'),
    /不会覆盖/,
  );
});

void test('cancelled, quota-failed and racing restores preserve data', async () => {
  const original = new MemoryStore();
  await unlockVault(original, '0042', 'setup');
  await appendLog(original, sample());
  const backup = exportEncryptedBackup(original);
  const destination = new MemoryStore();
  await assert.rejects(
    restoreEncryptedBackup(destination, backup, '0042', () => false),
    /取消/,
  );
  let checks = 0;
  await assert.rejects(
    restoreEncryptedBackup(destination, backup, '0042', () => ++checks < 2),
    /取消/,
  );
  assert.equal(destination.data.size, 0);
  ...
  checks = 0;
  await assert.rejects(
    restoreEncryptedBackup(changed, backup, '0042', () => {
      if (++checks === 2) changed.setItem(LEGACY_LOG_KEY, 'old-tab-write');
      return true;
    }),
    /不会覆盖/,
  );
  assert.equal(changed.getItem(VAULT_KEY), null);
  assert.equal(changed.getItem(LEGACY_LOG_KEY), 'old-tab-write');
});

void test('backup allowlist excludes unexpected plaintext fields in otherwise valid envelopes', async () => {
  const original = new MemoryStore();
  await unlockVault(original, '0042', 'setup');
  await appendLog(original, sample());
  const envelope = JSON.parse(original.getItem(VAULT_KEY));
  envelope.unexpected = 'secret-plaintext';
  envelope.records[0].unexpected = 'secret-plaintext';
  envelope.privateKey.unexpected = 'secret-plaintext';
  original.setItem(VAULT_KEY, JSON.stringify(envelope));
  const backup = exportEncryptedBackup(original);
  assert.ok(!backup.includes('secret-plaintext'));
  const incoming = JSON.parse(backup);
  incoming.vault.unexpected = 'secret-plaintext';
  const destination = new MemoryStore();
  await restoreEncryptedBackup(destination, JSON.stringify(incoming), '0042');
  assert.ok(!destination.getItem(VAULT_KEY).includes('secret-plaintext'));
});
