...
[Windows/macOS 自动检查](https://github.com/apcg2/emotion-vault/actions/workflows/windows.yml)验证安装、构建、磁盘读写/备份、接口安全、客户端失败处理、只读详情及对应系统启动脚本。测试只使用临时目录中的合成数据；这不等于已人工验证每种桌面安全弹窗或浏览器组合。
